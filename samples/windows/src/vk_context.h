...
public:
    ...
    // `enableValidation` is honoured only if the Khronos validation layer is
    // actually installed; a missing layer is a warning, never a failure.
    bool init(HWND window, bool enableValidation);
    void destroy();
    ...
    bool recreateSwapchain(HWND window);
